...
export default function NotificationsPage() {
  ...
  const fetchNotifications = async () => {
    try {
      const res = await fetch('http://localhost:3001/api/notifications?limit=100', {
  ...
  const clearAllRead = async () => {
    ...
    try {
      const res = await fetch('http://localhost:3001/api/notifications/clear-read', {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const getNotificationIcon = (notification: Notification) => {
    if (notification.type === 'alert_triggered' && notification.data?.alertType) {
      return notification.data.alertType === 'above' ? (
          ...
          <div className="flex items-center gap-3">
            {unreadCount > 0 && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-emerald-500 hover:bg-emerald-500/10 rounded-lg transition-colors"
              >
                        ...
                        <div className="flex items-center gap-4 mt-2">
                          ...
                          {notification.data.currentApr !== undefined && (
                            <span className="text-xs text-emerald-400">
                              Current: {notification.data.currentApr.toFixed(2)}%
                            </span>
                          )}
                        </div>
                      )}
