...
export default function NewAlertPage() {
  ...
  // Get unique assets and platforms
  const assets = [...new Set(aprData.map(d => d.asset))].sort();
  const platforms = [...new Set(aprData.map(d => d.platform))].sort();

  // Get current APR for selected asset/platform
  const currentApr = aprData.find(
    d => d.asset === asset && d.platform === platform
  )?.apr;
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400 mt-1">
            Get notified when APR rates change
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {asset && platform && (
              <div className="p-4 bg-gray-700/50 rounded-lg">
                <p className="text-sm text-gray-400">Current APR for {asset} on {platform}:</p>
                <p className="text-2xl font-bold text-emerald-400 mt-1">
                  {currentApr !== undefined ? `${currentApr.toFixed(2)}%` : 'N/A'}
                </p>
              </div>
            )}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  <div className="flex items-center justify-center space-x-2">
                    <ArrowUp
                      className={`h-5 w-5 ${
                        alertType === 'above' ? 'text-green-500' : 'text-gray-400'
                      }`}
                    />
                    <span
                      className={`font-medium ${
                        alertType === 'above' ? 'text-green-400' : 'text-gray-300'
                      }`}
                    >
                      Above
                    </span>
                ...
                >
                  <div className="flex items-center justify-center space-x-2">
                    <ArrowDown
                      className={`h-5 w-5 ${
                        alertType === 'below' ? 'text-red-500' : 'text-gray-400'
                      }`}
                    />
                    <span
                      className={`font-medium ${
                        alertType === 'below' ? 'text-red-400' : 'text-gray-300'
                      }`}
                    >
                      Below
                    </span>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-2">
                You&apos;ll be notified when the APR goes {alertType} this percentage
              </p>
            </div>
            ...
            {asset && platform && threshold && (
              <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                <p className="text-emerald-400 text-sm">
                  <strong>Preview:</strong> You&apos;ll receive a notification when{' '}
                  <span className="font-semibold">{asset}</span> APR on{' '}
                  <span className="font-semibold">{platform}</span> goes{' '}
                  <span
                    className={
                      alertType === 'above' ? 'text-green-400' : 'text-red-400'
                    }
                  >
                    {alertType} {threshold}%
                  </span>
