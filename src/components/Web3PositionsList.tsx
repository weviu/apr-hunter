...
export function Web3PositionsList({
  ...
  const formatTVL = (value: number, decimals = 2): string => {
  ...
  return (
    <div className="space-y-4">
      {groups.map((group) => (
        <div key={group.protocol} className="border border-gray-700 rounded-lg overflow-hidden">
          <button
          ...
          >
            <div className="flex items-center gap-3 flex-1">
              <div className={`${getProtocolColorClass(group.protocol)} w-3 h-3 rounded-full`} />
              ...
              <div className="text-right hidden sm:block">
                <p className="font-semibold text-white">{formatTVL(group.totalAmount, 0)}</p>
                <p className="text-sm text-blue-400">{group.averageApr.toFixed(2)}% avg APR</p>
              </div>
            </div>
