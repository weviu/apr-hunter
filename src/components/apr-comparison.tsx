...
export function AprComparison() {
  ...
  const aprAssetsQuery = useQuery({
    queryKey: ['apr-assets'],
    queryFn: () => api.get<{ data: { data: { symbol: string; name: string }[] } }>('/api/apr/assets'),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const assetOptions = useMemo(() => {
    const aprAssets =
      (aprAssetsQuery.data?.data?.data ?? []) as { symbol: string; name?: string }[];
    const allAssets =
      (allAssetsQuery.data?.data?.data ?? []) as { symbol: string; name?: string }[];
