...
type PriceDoc = { symbol: string; price: number; fetchedAt: string };
type SymbolMapDoc = { symbol: string; coingeckoId: string; fetchedAt: string };
...
async function fetchFromCoinGecko(symbols: string[]): Promise<Record<string, number>> {
  ...
  for (const [sym, id] of Object.entries(COINGECKO_IDS)) {
    if (data[id]?.usd !== undefined) out[sym] = data[id].usd;
  }
  return out;
}
...
async function getCached(db: Db, symbols: string[]): Promise<Record<string, number>> {
  ...
  const docs = await db
    ...
    .find({ symbol: { $in: symbols }, fetchedAt: { $gte: cutoff } })
    .toArray();
  const map: Record<string, number> = {};
  docs.forEach((d) => { if (typeof d.price === 'number') map[d.symbol] = d.price; });
  return map;
}
...
async function resolveMissingSymbolIds(db: Db, symbols: string[]) {
  const cutoff = new Date(Date.now() - MAP_TTL_MS).toISOString();
  const symbolSet = Array.from(new Set(symbols.map((s) => s.toUpperCase())));

  const cached = await db
    .collection<SymbolMapDoc>(MAP_COLLECTION)
    .find({ symbol: { $in: symbolSet }, fetchedAt: { $gte: cutoff } })
    .toArray();
  cached.forEach((doc) => { if (doc.coingeckoId) COINGECKO_IDS[doc.symbol.toUpperCase()] = doc.coingeckoId; });
  ...
  for (const sym of stillMissing) {
    try {
      ...
      if (id) {
        ...
        await db.collection<SymbolMapDoc>(MAP_COLLECTION).updateOne(
          { symbol: sym },
          { $set: { symbol: sym, coingeckoId: id, fetchedAt: new Date().toISOString() } },
          { upsert: true },
        );
      }
    } catch { /* ignore */ }
...
/** Returns USD prices for the given symbols, using a 60-second MongoDB cache. */
export async function getPrices(symbols: string[]): Promise<Record<string, number>> {
  ...
  try {
    ...
    console.error('[prices] CoinGecko fetch failed', e);
    return cached;
  }
}
